...
build_git_ios()
{
  ...
  echo "build external: $build_for_external, working in $tempbuilddir"
  ...
  if echo $library|grep '\.framework$'>/dev/null ; then
    ...
    for dep in $embedded_deps ; do
      ...
      elif test -d "$scriptpath/../Externals/$dep" ; then
        mv "$scriptpath/../Externals/$dep" "$name-$version"
      else
